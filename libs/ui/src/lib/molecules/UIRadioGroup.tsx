...
export type UIRadioGroupProps = {
  value: any;
  items: Array<{ value: any; text: string; disabled?: boolean; otherInput?: boolean }>;
  label?: any;
  name?: string;
  error?: boolean;
...
const UIRadioGroup = (props: UIRadioGroupProps) => {
  const { value, label, name, items, position, error } = props;
  const definedValue = items?.filter((item: any) => !item.otherInput).map((filterd: any) => filterd.value) || [];
  const initValue = definedValue.indexOf(value) < 0 && value != null ? DEFAULT_OTHER_VALUE : value;
  ...
  const [isDisableOther, setDisableOther] = useState(value == null || definedValue.indexOf(value) > 0);
  ...
  return (
    <FormControl sx={style} className="ui-radio-group-root">
            ...
            <>
              {item.otherInput ? (
                <FormControlLabel
                  ...
                  label={
                    <Box display="flex" flexDirection="row" columnGap="6px" alignItems="center">
                      <Typography fontSize="14px">{item.text || 'その他'}</Typography>
                      <TextField
                        value={otherValue}
                        size="small"
